...
class MyDataCollector(gym.Wrapper):
    ...
    def __init__(
        ...
        episode_data_callback: Type[EpisodeDataCallback] = EpisodeDataCallback,
        ...
        self._episode_data_callback = episode_data_callback()
        ...
        self._buffer: Optional[MyEpisodeBuffer] = None
        self._episode_id = 0
        self._reset_storage()
    ...
    def step(
        ...
        step_data: MyStepData = self._step_data_callback(
            env=self.env,
            obs=obs,
            info=info,
            action=action,
        ...
        if "masks" not in step_data.keys():
            step_data["masks"] = self.create_dumb_masks(step_data["observation"])

        self._buffer = self._buffer.add_step_data(step_data)

        if step_data["terminated"] or step_data["truncated"]:
            self._episode_data_callback(self._buffer)
            self._storage.update_episodes([self._buffer])
            self._episode_id += 1
            self._buffer = MyEpisodeBuffer(
                id=self._episode_id,
    ...
    def reset(
        ...
        *,
        ...
        self._buffer = MyEpisodeBuffer(
            ...
            options=options,
            observations=step_data["observation"],
            # masks=self.create_dumb_masks(step_data["observation"]),
            infos=step_data["info"] if self._record_infos else None,
        )
        return obs, info

    def create_dumb_masks(self, obs: np.ndarray) -> np.ndarray:
        """Create dummy masks for the observations."""

        return np.ones((obs.shape[0], obs.shape[1]), dtype=np.uint8)
